...
//  glui->add_checkbox("Sub-Stepping", &settings.enable_sub_stepping);
  ...
  new GLUI_Column( glui, false );
  GLUI_Panel* drawPanel =	glui->add_panel("Instructions:");
  glui->add_statictext_to_panel(drawPanel,"1).Use spacebar to make you jump :P ");
...
//  glui->add_checkbox_to_panel(drawPanel, "Profile", &settings.draw_profile);
  ...
  new GLUI_Column( glui, false );
  GLUI_Panel* drawPane =	glui->add_panel("Copyrights:");
  glui->add_statictext_to_panel(drawPane,"Mr.Jump, 1Button SARL");
...
int main(int argc, char** argv)
{
  ...
  char title[50];
  sprintf(title, "CS251 first Project. Running on Box2D %d.%d.%d", b2_version.major, b2_version.minor, b2_version.revision);
